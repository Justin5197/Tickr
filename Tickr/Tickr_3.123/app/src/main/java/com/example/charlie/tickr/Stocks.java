...
public class Stocks extends AppCompatActivity implements Runnable{
    ...
    public Double getStockPrice(String url) throws MalformedURLException, IOException, JSONException {
        URL reqURL = new URL("https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol=" + this.stockName + "&interval=1min&apikey=" + APIKEY); //the URL we will send the request to
    ...
    public void run() {
        try {
            value = getStockPrice(this.stockName);
        } catch (IOException e) {
            e.printStackTrace();
